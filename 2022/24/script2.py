...
options = [(0, 1, False, False)]
step = 0
while True:
	new_options = set()
	for option in options:
		(r, c, hit_end, hit_start) = option
		if not hit_end and (r == map.len - 1 and c == map[r].len - 2):
			hit_end = True
		if (hit_end and not hit_start) and r == 0 and c == 1:
			hit_start = True
		if (hit_end and hit_start) and (r == map.len - 1 and c == map[r].len - 2):
			sys.exit()
		map = evolved_map(step + 1)
		for neighbor in neighbors5(map, r, c):
			if not map[neighbor[0]][neighbor[1]].len:
				# "n", neighbor
				new_options.add((*neighbor, hit_end, hit_start))
		# print_map(map)
	options = new_options
	step += 1
	step
