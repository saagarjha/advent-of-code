...
def do_move(stacks, source, dest, count):
	stacks[source] = stacks[source].filter(_0)
	stacks[dest].append(stacks[source][-count:])
	stacks[dest] = stacks[dest].flatten().filter(_0)
...
for move in moves:
	...
	stacks.str_join("\n")
	count, location
	do_move(stacks, location[0] - 1, location[1] - 1, count)
	stacks.str_join("\n")
	print()
