		...
		for x_flip in [-1, 1]:
			for y_flip in [-1, 1]:
				for z_flip in [-1, 1]:
					...
					rotations.append((s, (permutation, [x_flip, y_flip, z_flip])))
	return rotations
...
while remaining.len:
	...
	for i in remaining.copy():
		...
		if done:
			break
		for rotation1, mapping in all_rotations:
			if done:
				break
			for rotation2, _ in rotations(scanners[i]):
				(good, new, offset) = matches(rotation1, rotation2)
				if good:
					all_points = new
					remaining.remove(i)
					done = True
					...
					for location in locations:
						...
						locations[location] = (l[mapping[0][0]] * mapping[1][0], l[mapping[0][1]] * mapping[1][1], l[mapping[0][2]] * mapping[1][2])
					locations[i] = offset
					break
locations
max_distance = 0
for l1 in locations:
	...
	for l2 in locations:
		l2 = locations[l2]
		max_distance = max(max_distance, abs(l1[0] - l2[0]) + abs(l1[1] - l2[1]) + abs(l1[2] - l2[2])) 
print(max_distance)
